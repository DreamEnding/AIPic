...
const { createTaskDatabase, acknowledgeTask } = require('../storage/task-db');
// Adapted for AIPic from FlyReq Image Studio, commit 9e286d970a67b14465cff35e03a9c5c7712a504c.
// Original and adapted backend are licensed under AGPL-3.0; see LICENSE and README.md.

const { safeFetch } = require('../security/outbound-url');
const { MAX_BODY_BYTES, selectUpstream, validateImages, validateProxyBody } = require('../security/policy.cjs');
...
const GLOBAL_TASK_CONCURRENCY = 4;
const MAX_PARALLEL_COUNT = 4;
const MAX_IMAGE_PARALLEL_COUNT = 4;
const DEFAULT_LIMIT_CONFIG = {
  maxQueueSize: 16,
  rateLimitWindowMs: 60 * 1000,
  maxRequestsPerIp: 20,
  maxRequestsPerApiKey: 20,
  maxPendingTasksPerIp: 4,
  maxPendingTasksPerApiKey: 4,
...
function installTimestampedConsole() {
  for (const method of ['log', 'info', 'warn', 'error']) {
    ...
    console[method] = (...args) => write(`[${getLogTimestamp()}]`, ...args.map(redact));
  }
}
...
function resolveAndLogOutboundBaseUrl(requestType, protocol, baseUrl, env = getRuntimeEnv()) {
  ...
  console.info(`[base-url-rewrite] 状态=${status} 请求=${requestType} 协议=${protocol} 原始Base URL=${getSafeUrlLabel(details.originalBaseUrl)} 最终Base URL=${getSafeUrlLabel(details.baseUrl)} 映射规则数=${details.rewriteCount}`);
  return details;
}
...
function logBaseUrlRewriteConfiguration() {
  ...
  const mappings = rewrites
    .map(rewrite => `${getSafeUrlLabel(rewrite.from)}=>${getSafeUrlLabel(rewrite.to)}`)
    .join(' | ');
...
const HOSTNAME = process.env.AIPIC_HOST || process.env.HOSTNAME || '127.0.0.1';
const DB_PATH = process.env.FLYREQ_TASK_DB || path.join(BACKEND_DIR, 'flyreq-tasks.sqlite');
...
const STATIC_DIR = process.env.AIPIC_STATIC_DIR || path.join(BACKEND_DIR, '..', 'dist');
const IMAGE_DIR = process.env.FLYREQ_IMAGE_DIR || path.join(BACKEND_DIR, 'flyreq-images');
const VIDEO_DIR = process.env.FLYREQ_VIDEO_DIR || path.join(BACKEND_DIR, 'flyreq-videos');
const taskRefImages = new Map();
const taskRefMasks = new Map();
...
function getClientIp(req) {
  const forwardedFor = undefined; // Forwarded headers are untrusted without an authenticated proxy.
...
function sendJson(res, statusCode, body, extraHeaders = {}) {
  ...
  res.end(JSON.stringify(statusCode >= 400 ? redact(body) : body));
}
...
const MAX_REQUEST_BODY_BYTES = MAX_BODY_BYTES;
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      ...
      if (bytes > MAX_BODY_BYTES) {
        ...
        chunks.length = 0;
        req.resume();
        reject(createHttpError(413, 'PAYLOAD_TOO_LARGE', '请求体过大'));
      } else chunks.push(chunk);
    });
    req.on('end', () => { if (!failed) resolve(Buffer.concat(chunks)); });
    req.on('error', reject);
    req.on('aborted', () => reject(createHttpError(400, 'REQUEST_ABORTED', '请求已中断')));
  });
}
...
function readVideoMultipartBody(req) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      ...
      if (totalBytes > MAX_BODY_BYTES && !failure) {
        failure = createHttpError(413, 'PAYLOAD_TOO_LARGE', '请求体过大');
        req.unpipe(busboy);
        busboy.destroy(failure);
        req.resume();
        reject(failure);
      }
    });
...
async function normalizeVideoTaskPayload(fields, files) {
  ...
  const payload = {
    ...
    baseUrl: selectUpstream(getRuntimeEnv(), fields.providerId, normalizeProtocolBaseUrl(protocol, fields.baseUrl)),
...
function validateCreatePayload(body) {
  ...
  if (body.providerId) body.baseUrl = selectUpstream(getRuntimeEnv(), body.providerId, body.baseUrl);
  ...
  // Compatible Grok endpoints accept the same arbitrary-size Images API as GPT.
  if (body.imageApiFlavor === 'openai') delete body.imageApiFlavor;
  ...
  let upstreamUrl;
  try { upstreamUrl = new URL(body.baseUrl); } catch { throw new Error('API 基础地址无效'); }
  body.baseUrl = selectUpstream(getRuntimeEnv(), body.providerId, body.baseUrl);
  validateImages(body);
  if (body.imageApiFlavor === 'xai-imagine' && upstreamUrl.hostname !== 'api.x.ai') delete body.imageApiFlavor;
  if (body.customSize && !parseImageSize(body.customSize)) throw new Error('自定义图片尺寸无效');
  if (body.mask !== undefined && (!body.mask || typeof body.mask.data !== 'string' || !body.mask.data)) throw new Error('蒙版图片数据无效');
  if (body.gptImageOutputCompression !== undefined && (!Number.isInteger(body.gptImageOutputCompression) || body.gptImageOutputCompression < 0 || body.gptImageOutputCompression > 100)) throw new Error('图片压缩质量参数无效');
  if (body.gptImageModeration !== undefined && !['auto', 'low'].includes(body.gptImageModeration)) throw new Error('图片审核参数无效');
  if (body.responseFormat !== undefined && !['url', 'b64_json'].includes(body.responseFormat)) throw new Error('图片响应格式无效');
  if (body.streamPartialImages !== undefined && (!Number.isInteger(body.streamPartialImages) || body.streamPartialImages < 0 || body.streamPartialImages > 3)) throw new Error('流式预览数量无效');
...
function buildTaskRequestForDb(body, parallelCount = body.parallelCount, promptVariants = body.promptVariants) {
  return {
    ...
    gptImageOutputCompression: body.gptImageOutputCompression,
    gptImageModeration: body.gptImageModeration,
 ...
 */
function registerTaskRuntimeState(taskId, apiKey, images, source, mask) {
  apiKeys.set(taskId, apiKey);
  taskRefImages.set(taskId, images);
  if (mask) taskRefMasks.set(taskId, mask);
...
function createTask(body, req) {
  ...
  registerTaskRuntimeState(taskId, body.apiKey, body.images, source, body.mask);
...
function createTaskBatch(body, req) {
  ...
  for (const task of tasks) {
    registerTaskRuntimeState(task.taskId, body.apiKey, body.images, source, body.mask);
...
function resolveGptImageRequestSize(request) {
  // AIPic already computes provider-specific pixel sizes; never clamp or round them.
  const customSize = parseImageSize(request.customSize);
  if (customSize) return `${customSize.width}x${customSize.height}`;
...
async function fetchWithTimeout(url, init = {}) {
  ...
  try {
    ...
    // Response headers are not completion: retain timeout/abort through the body.
...
function drainVideoQueue() {
  ...
  while (videoQueue.length > 0 && activeVideoCount < maxConcurrency && activeCount + activeVideoCount < getMaxServerConcurrency()) {
...
function drainQueue() {
  ...
  while (queue.length > 0) {
    ...
    const imageSlots = Math.min(req?.parallelCount || 1, maxConcurrency);

    const fitsWithinLimit = activeCount + activeVideoCount + imageSlots <= maxConcurrency;
    if (!fitsWithinLimit) break;
...
async function generateSingleImage(apiKey, request, taskId, index) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(new DOMException('Image generation timed out after 30 minutes', 'TimeoutError')), REQUEST_TIMEOUT_MS);
  let usesSse = false;
  try {
    ...
    for (let subIdx = 0; subIdx < expanded.length; subIdx++) {
      ...
      if (img.startsWith('URL:')) {
        ...
        const result = await downloadUrlToDisk(taskId, index, subIdx, remoteUrl, { apiKey, request: requestForImage, signal: controller.signal });
        diskRefs.push(`URL:${result.httpUrl}`);
      } else {
        if (img.length > Math.ceil(DEFAULT_REMOTE_IMAGE_MAX_BYTES / 3) * 4) throw new Error('生成图片超过大小限制');
...
async function runTask(taskId) {
  ...
  request.mask = taskRefMasks.get(taskId);
  db.prepare("UPDATE tasks SET status = 'processing' WHERE id = ?").run(taskId);
  broadcastTask(taskId);
  broadcastQueueStatus();
  ...
  const itemResults = [];
  const concurrency = getMaxServerConcurrency();
  for (let start = 0; start < request.parallelCount; start += concurrency) {
    itemResults.push(...await Promise.allSettled(
      Array.from({ length: Math.min(concurrency, request.parallelCount - start) }, (_, index) =>
        generateSingleImage(apiKey, request, taskId, start + index)
      )
    ));
  }
...
function isTerminalTaskStatus(status) {
  return status === TASK_STATUS.COMPLETED
    || status === TASK_STATUS.INTERRUPTED || status === TASK_STATUS.FAILED
    || status === TASK_STATUS.CANCELLED
    || status === 'expired';
}
...
const { ensureImageDir, saveImageToDisk, ensureVideoDir, getVideoExtension, findTaskVideoFile, deleteTaskVideoFile, sendVideoFile, deleteTaskImageFiles } = createImageStorage({ fs, IMAGE_DIR, path, VIDEO_DIR });
const { enforceGeneratedImageLayout, downloadUrlToDisk } = createImageService({ parseImageSize, getGptImageSize, sharp, parseBooleanEnv, getRuntimeEnv, shouldAuthorizeRemoteImageDownload, fetchWithTimeout, getSafeUrlLabel, parseIntegerEnv, DEFAULT_REMOTE_IMAGE_MAX_BYTES, saveImageToDisk });
const { parseJsonSafely, isLikelyHtmlResponse, getMessageFromPayload, getErrorMessageFromPayload, isImageEventStreamResponse, notifyImageSseResponse, parseGptImageResponse, requestGptImage } = createOpenaiAdapter({ validateEnumValue, GPT_IMAGE_QUALITIES, GPT_IMAGE_STYLES, GPT_IMAGE_BACKGROUNDS, GPT_IMAGE_OUTPUT_FORMATS, DEFAULT_GPT_IMAGE_ADVANCED_PARAMS, readImageEventStream, formatImageUpstreamError, resolveFlyreqApiBaseUrl, appendProtocolApiPath, getImageUpstreamLogOptions, logImageRequestUrl, logImageUpstreamRequest, fetchWithTimeout, logImageUpstreamResponse });
const { requestXaiImagineImage } = createXaiAdapter({ getXaiImagineEndpoint, appendProtocolApiPath, getImageUpstreamLogOptions, logImageRequestUrl, XAI_IMAGINE_MAX_RETRIES, waitForXaiImagineRequestSlot, createXaiImagineRequestInit, logImageUpstreamRequest, fetchWithTimeout, logImageUpstreamResponse, isImageEventStreamResponse, notifyImageSseResponse, parseGptImageResponse, getRetryAfterDelayMs, delay });
const { generateFlyreqGeminiImage } = createGoogleAdapter({ resolveFlyreqApiBaseUrl, appendProtocolApiPath, getImageUpstreamLogOptions, logImageRequestUrl, logImageUpstreamRequest, fetchWithTimeout, logImageUpstreamResponse, getUpstreamHttpErrorPrefix, isLikelyHtmlResponse, parseJsonSafely, getErrorMessageFromPayload });
const generateFlyreqImage = require('./provider-service').createProviderService({ requestGptImage, requestXaiImagineImage, generateFlyreqGeminiImage, resolveGptImageRequestSize, resolveAndLogOutboundBaseUrl, resolveFlyreqApiBaseUrl });
const { handleAipicProxy } = createProxyRoutes({ parseBooleanEnv, getRuntimeEnv, sendJson, selectUpstream, readRawBody, validateProxyBody, fetchWithTimeout, pipeline, Readable, normalizeError });
const { handleImages } = require('../routes/images').createImageRoutes({ sendJson, path, fs, IMAGE_DIR, pipeFileToResponse, getContentType, findTaskVideoFile, sendVideoFile });
const { handleApi } = createTaskRoutes({ ackTask: taskId => acknowledgeTask(db, taskId), sendJson, getQueueStats, path, fs, buildPlatformManifest, resolvePlatformBranding, getRuntimeEnv, resolveImageModelKeyGuide, resolveImagePresetModelIds, resolveDefaultImageModelConfig, resolveDefaultVideoModelConfig, resolveVideoWorkspaceConfig, resolveVideoProtocolConfig, readJsonBody, hashPromptGalleryPassword, selectUpstream, resolveAndLogOutboundBaseUrl, appendProtocolApiPath, fetchWithTimeout, normalizeError, stripProtocolVersionSuffix, readVideoMultipartBody, normalizeVideoTaskPayload, createVideoTaskBatch, db, serializeTask, createVideoTask, cancelVideoTask, createTaskBatch, createTask, isHttpError, sendHttpError, __dirname: BACKEND_DIR });
const { initDatabase } = createTaskDatabase({ db, TASK_STATUS, TASK_TTL_MS, deleteTaskImageFiles });
initDatabase();
...
// Keep the existing AIPic proxy URL usable for Responses and custom API modes.
...
async function shutdownTasks() {
  ...
  while ((activeCount || activeVideoCount) && Date.now() < deadline) await delay(100);
  await flushDailyFileLogs();
  if (!activeCount && !activeVideoCount) db.close();
}
module.exports = { handleImages, shutdownTasks, setupWebSocketServer, HOSTNAME, PORT, sendJson, db, storageReady, getRuntimeEnv, createHttpError, handleAipicProxy, handleApi, serveStatic, handleServerListenError, flushDailyFileLogs };
