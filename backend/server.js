...
const STATIC_DIR = process.env.AIPIC_STATIC_DIR || path.join(__dirname, '..', 'dist');
...
const MAX_REQUEST_BODY_BYTES = 128 * 1024 * 1024; // AIPic 4K references and masks may exceed 10 MB after Base64 encoding.
...
function validateCreatePayload(body) {
  ...
  if (!['http:', 'https:'].includes(upstreamUrl.protocol)) throw new Error('API 基础地址必须为 HTTP 或 HTTPS');
...
function normalizeAipicProxyBaseUrl(value) {
  const raw = String(value || '').trim();
  if (!raw) return '';
  try {
    ...
    const baseSegments = versionIndex >= 0 ? segments.slice(0, versionIndex + 1) : [...segments, 'v1'];
    return `${url.origin}/${baseSegments.join('/')}`;
  } catch {
    return '';
  }
}
...
async function handleAipicProxy(req, res, parsedUrl) {
  ...
  try {
    ...
    for (const [name, value] of Object.entries(req.headers)) {
      ...
      if (['host', 'origin', 'x-aipic-upstream', 'x-aipic-proxy-stream', 'x-aipic-timeout-seconds'].includes(name)) continue;
    ...
    if (!res.headersSent && !res.destroyed) {
      sendJson(res, /timeout|timed out/i.test(String(error?.message)) ? 504 : 502, { error: normalizeError(error) }, corsHeaders);
